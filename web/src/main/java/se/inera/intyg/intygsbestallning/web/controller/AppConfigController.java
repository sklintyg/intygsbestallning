...
public class AppConfigController {
    public static final String APPCONFIG_REQUEST_MAPPING = "/public-api/appconfig";
    private BuildProperties buildProperties;
    private Environment environment;

    public AppConfigController(BuildProperties buildProperties, Environment environment) {
        this.buildProperties = buildProperties;
        this.environment = environment;
    }

    @GetMapping(path = APPCONFIG_REQUEST_MAPPING, produces = MediaType.APPLICATION_JSON_VALUE)
    ResponseEntity getAppConfig() {
        ...
        var versionInfo = new VersionInfo(applicationName, buildVersion, buildTimestamp, activeProfiles);

        return ResponseEntity.ok(new GetAppConfigResponse(versionInfo, SecurityConfig.SAML_LOGIN_URL));
    }
}
